...
辱骂、骚扰或其他不可接受的行为可通过下列渠道向负责监督的社区领袖报告：

- **GitHub Security Advisories（首选，私密）**：
  <https://github.com/trx-0833/EngramNote/security/advisories/new>
- **GitHub Issues（次选）**：<https://github.com/trx-0833/EngramNote/issues>

⚠️ 本仓库**没有公开邮箱**。若走 Issues，请**不要在公开 issue 里写出事件细节、
当事人信息或任何可识别内容**——只说明"需要私下沟通行为准则相关事宜"，
维护者会把渠道转成私密方式后再请你补充。
...
中文版参见 <https://www.contributor-covenant.org/zh-cn/version/2/1/code_of_conduct/>。
...
> **本项目的补充说明（不是公约原文）**
>
> - 本项目是**单人维护**的自托管项目，维护者即
>   `CODEOWNERS` 里登记的那一位，没有独立的行为准则委员会；
> - 与技术问题相关的沟通方式见 `CONTRIBUTING.md`；
>   与安全漏洞相关的私密渠道见 `SECURITY.md`（**优先走 Security Advisories**）；
> - 请特别避免在公开渠道粘贴**真实笔记内容、上传文件、数据库文件或 `backend/.env`**
>   —— 它们既是隐私，也可能包含凭据。
