...
| 异步任务 | Celery + 文件系统 broker | 无需 Redis |
| AI 理解 | DeepSeek / GLM API | 通过 OpenAI 兼容接口调用 |
| 文档解析 | MinerU | PDF → Markdown（保留 LaTeX 公式、表格） |
| 嵌入模型 | BGE-M3 (BAAI/bge-m3) | 多语言嵌入，ModelScope 优先下载 |
...
│   │   │   ├── llm_service.py         # DeepSeek/GLM 调用
...
│   ├── src/
│   │   ├── api/                 # API 请求封装
│   │   ├── components/          # 通用组件（DiffView, CleaningPanel 等）
│   │   ├── pages/               # 页面组件（16 个页面）
...
```
上传资料 → MinerU 转 Markdown → AI 清洗（去噪+去重）→ AI 理解（摘要+知识点+题目）
                                                              ↓
                                                        知识图谱构建
                                                              ↓
学习评估 ← SM-2 复习 ← RAG 问答 ← 知识卡片库
```

1. **上传** → PDF/图片/Office/音视频 自动转换为 Markdown
2. **清洗** → 规则去噪 + BGE-M3 向量去重，生成干净副本（三视图对比）
