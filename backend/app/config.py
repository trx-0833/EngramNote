...
class Settings(BaseSettings):
    ...
    # 访问令牌过期时间（分钟），默认 1440 分钟 = 24 小时
    #
    # ⚠️ 阶段 6.3 起访问令牌**仍然是无状态的**，因此这个值直接等于
    # "登出/撤销之后，已泄露的那枚访问令牌还能用多久"的上界。
    # 本轮刻意**不改**这个值：把它从 24 小时砍到 15 分钟会改变所有在用
    # 客户端的续期频率（前端刚接上刷新链路，需要先观察一轮真实表现），
    # 缩短 TTL 属于独立的、需要单独验证的变更（见 docs/overhaul-plan.md 附录 BA.3）。
    jwt_expire_minutes: int = 1440
    # 刷新令牌有效期（天），默认 30 天（阶段 6.3）
    #
    # 它是"用户多久必须重新输一次密码"的上界：只要在有效期内且未被撤销，
    # 轮换链就能一直续下去。30 天与常见消费级产品的"记住我"档位一致；
    # 调大 → 更少重登，但一枚泄露的刷新令牌窗口更长；
    # 调小 → 更频繁重登。撤销（登出/重放检测）不受它影响，随时生效。
    ...
    # ---- 密码哈希（阶段 6.1）----
    # bcrypt cost（轮数）。每 +1 让计算量翻倍：10≈50ms、12≈200ms、14≈800ms。
    #
    # 显式配置而不是用库默认值：默认值会随 bcrypt 版本变化，而 cost 直接决定
    # 离线爆破成本 —— 它应当是**部署方明确选择**的参数，不是库的偶然取值。
    # 12 是 2026 年的常见下限（OWASP 建议 ≥10，并随硬件提升）。
    # 提升这个值不会让旧哈希失效：cost 写在哈希串里，校验按各自记录的值计算。
    bcrypt_rounds: int = 12

    # ---- 上传安全护栏（阶段 6.4；0 均表示不启用该条） ----
    #
    # 这三条与上面的"容量配额"互补：容量管总量，它们管**单个文件与条目数**
    # 的异常形态 —— 恶意或畸形的输入首先表现为"某个数字离谱"。
    #
    # PDF 页数上限：页数直接决定 MinerU 与后续 LLM 的工作量。
    # 2000 页远超正常学习资料（一本教科书通常 <1000 页），
    # 而一本 3000 页的书会把理解管道拖成不可控的耗时与成本。
    max_pdf_pages: int = 2000
    # Office 文档（docx/pptx/xlsx 都是 ZIP 容器）解压后的体积上限（MB）。
    # 这类文档正常解压后远小于 500MB；超过说明它是"压缩炸弹"或畸形文件。
    max_archive_uncompressed_mb: int = 500
    # 允许的最大压缩比（解压后/压缩后）。正常 Office 文档一般在 10:1 以内，
    # 100:1 以上基本只有刻意构造的炸弹能做到（42KB → 4GB 是经典比例）。
    max_archive_compression_ratio: int = 100
    # 压缩包内文件条目数上限（防止"几百万个空文件"这类形态）
    max_archive_entries: int = 5000
    # 单用户笔记数上限。容量配额（MB）挡不住"传一万个小文件"：
    # 每个都要建笔记、进转换队列、占 inode。默认值取得很宽（真实使用量级在千以内），
    # 它是**护栏**而不是产品限制 —— 真到上限时提示用户清理回收站。
    ...
    backup_keep: int = 14
    #: 对象存储快照保留份数。比数据库快照少得多是**有意的**：
    #: 每份含全部上传文件（当前约 82 MB），而"误删文件"通常几天内就会被发现，
    #: 保留 3 份足够覆盖，同时把 `_backup/` 的占用压在 ~250 MB 量级。
    storage_backup_keep: int = 3
