...
test.describe('全链路（真实后端 + 真实 LLM）', () => {
  ...
  test('④ 复习：提交答案并推进调度', { tag: '@chain' }, async ({ page }) => {
    ...
    if (await textarea.count()) {
      await textarea.first().fill('顶层油温正常运行不得超过 85 摄氏度，达到 95 摄氏度必须降低负荷。')
  ...
  test('⑤ 问答：SSE 真的逐段吐出 token', { tag: '@chain' }, async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      w.__sse = { events: 0, tokens: [], meta: null, sources: null, done: false, error: null, status: 0 }
    ...
    const secondVisible = cleanAnswerText(await expectAnswerCardText(page, question), question).length
    ...
    const providerText = (
      await page.getByText(/由 (DeepSeek|GLM) 提供支持/).innerText()
    ).trim()
  ...
  test('⑥ LLM 记账 + 隔离校验：真的调了模型，且没有碰真实库', { tag: '@chain' }, async ({
    page,
  }) => {
