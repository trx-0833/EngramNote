...
> **状态：过程记录** ｜ 记录时间：2026-09-14（S1→S3b 逐轮追加）｜ 权威性：不是现状依据（契约现状以 `backend/openapi.json` 与 `frontend/src/api/generated/` 为准）
...
> 392 545 字节 / 102 / 118 / 181 / `c3adfe8be0a8`，阶段 0.10 之后变成
> 397 598 字节 / 103 / 119 / 184 / `9a633977af6e`（完整记录见 **§12**）；
> 此后每动一次契约它都会再变（`/ready` 加索引积压 → 398 998 / 185 / `6c6de4ffc9a7`；
> docstring 的 `Raises:` 批量更正 → 402 798 / 185 / `2e0f1a88eb49`）。
>
> **这三组以上数字只作历史对照**：字节数与 sha256 会随**任何一次契约改动**变化，
> 引用"当前值"时请以 `python backend/scripts/dump_openapi.py --check` 的输出为准，
> 不要抄这里的数字（抄了就会在下一次改动时变成假话）。
> —— 所以这里**不再逐轮追加**新的指纹，只留几组"当时的"作对照。
