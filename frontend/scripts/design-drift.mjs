...
const BASELINE = {
   ...
   *
   * 这条基线设于**阶段 E 全部批次落地之后**（F2 的收口动作）—— 计划 §10 的 F2 行
   * 写的就是"基线留到 A–E 完成后设"：在视觉整改跑完之前设基线，等于把整改过程中
   * 必然会动的两个数字钉死，门禁会在每一批都红一次，那它就退化成了噪音。
   *
   * 这两个数字**不是目标，是水位线**：门禁只拒绝"比它更高"，不要求往下压。
   * 想把水位降下来是单独一轮的事（存量 228 处硬编码色值与 408 处非令牌字号
   * 都是按既定决策"不动存量外观"保留下来的），届时**显式改这里的数字并写明理由**。
   */
...
const PENDING_WIRING = {
  ...
  // （`--z-dropdown` 曾登记在这里：批次 E6 给四档自评的 sticky 底栏加 `z-index` 时接了线）
  '--z-base': 'D2 的迁移只用到 --z-modal；这三个留待真有「元素需要显式层级」时使用',
  '--z-drawer': '同上',
  '--z-toast': '同上，待 D2',
  // ── 规范预留（**不是欠账**，是刻意留给新代码的命名）──
  // （`--space-2xs` / `--text-xs` / `--text-sm` / `--text-sm-alt` / `--text-base` /
  //   `--text-2xl` 曾登记在这里：阶段 E 逐页精修把"规范预留"这几个字兑现了 ——
  //   E5 的题库表格、E8 的 9 页、E1 的统计卡陆续真正用上了它们，于是过期自检
  //   把它们报了出来。按本表的规矩删掉：这张表要始终等于"当前真实的欠账"。）
  '--text-2xs':
