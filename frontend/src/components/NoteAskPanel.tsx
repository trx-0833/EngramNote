...
export default function NoteAskPanel({
  ...
  //: SSE `meta` 里的检索状态（`full_vector` / `hybrid` / `bm25_only`），
  //  由 `utils/retrievalNotice.ts` 翻译成给用户看的一句话
      ...
      await parseSSEStream(
        reader,
        {
          onEvent: (eventType, data) => {
            ...
            if (eventType === 'meta') {
              // 首事件：记录 LLM 提供商标识与**检索状态**
              // （`retrieval_status` 在契约里写明是"供前端展示降级提示"；
              //  本组件此前只取了 provider，于是"刚导入的笔记还没建向量索引"
              //  这种最常发生的降级在这里**完全看不见** —— 见附录 BN.4）
              setProvider(String(payload.provider || ''));
              setRetrievalStatus(String(payload.retrieval_status || ''));
              return;
            }
            if (eventType === 'token') {
  ...
  const panelLeft =
    maxLeft > minLeft ? Math.min(Math.max(panelPos.x, minLeft), maxLeft) : window.innerWidth / 2; // 视口比面板还窄：居中，由 maxWidth 保证不溢出
  ...
  const selectedTextPreview =
    initialText.length > 60 ? `${initialText.slice(0, 60)}...` : initialText;
        ...
        {mode === 'input' ? (
          <>
            <textarea
              ...
              rows={3}
            />
            <p className={styles.askAiSelectedHint}>
              基于当前笔记选中文本：「{selectedTextPreview}」
            </p>
          ...
          <>
            ...
            <div className={styles.askAiActions}>
              {streaming ? (
                <button className="btn btn-danger" onClick={stopActiveStream}>
                  停止生成
                </button>
              ) : (
                <>
                  <button className="btn btn-secondary" onClick={handleReask}>
                    重新提问
                  </button>
            ...
            {provider && (
              <p className={styles.askAiProvider}>
                由 {provider === 'glm' ? 'GLM' : 'DeepSeek'} 提供支持
              </p>
            )}
            {retrievalNotice(retrievalStatus) && (
              <p className={styles.askAiRetrievalNotice}>{retrievalNotice(retrievalStatus)}</p>
            )}
          </>
        )}
      </div>
