...
describe('ReviewProgress 的进度公式', () => {
  it('当前项未结账时只算已完成的那些', () => {
    const { container } = render(<ReviewProgress index={2} total={5} done={false} label="3 / 5" />);
    expect(fillWidth(container)).toBe('40%');
  });

  it('★ 当前项已结账时把它计入进度（少算这一张就是四页各自漂移过的地方）', () => {
    const { container } = render(<ReviewProgress index={2} total={5} done label="3 / 5" />);
    expect(fillWidth(container)).toBe('60%');
  });

  it('末项结账后是 100%', () => {
    const { container } = render(<ReviewProgress index={4} total={5} done label="5 / 5" />);
  ...
  it('总数缺失时给 0% 而不是 NaN%（"渲染坏了"比"没有进度"糟得多）', () => {
    const { container } = render(<ReviewProgress index={0} total={0} done={false} label="0 / 0" />);
...
describe('ReviewProgress 的两种排版', () => {
  it('一行式：计数在左、补充计数在右', () => {
    render(<ReviewProgress index={0} total={3} done={false} label="1 / 3" trailing="0/0 正确" />);
