...
import { normalizeGraphData, normalizeStats, normalizeSubgraph, normalizeSuggestions } from './normalize'
...
describe('normalizeStats（对象响应里的数组字段）', () => {
  it('(a) 分布字段被包成 {items:[…]}：退化成空分布并报出字段名', () => {
    const result = normalizeStats(makeStats({ relation_type_distribution: { items: [] } as never }))
...
describe('normalizeSubgraph（对象响应里的数组字段）', () => {
  ...
  it('(c) 正常响应：中心节点与邻居原样返回，一条漂移都不报', () => {
    ...
    const result = normalizeSubgraph({ center_node: node, neighbor_nodes: [node], edges: [] } as never)
