...
export default function Dashboard() {
  ...
  async function fetchRecent() {
    ...
    let failedCount = 0;
    try {
      const [notesRes, statsRes, reportRes, trendRes, weakRes, goalsRes, planRes] =
        await Promise.all([
          getNotes(1, 5),
  ...
  return (
    <div className="page-enter">
      ...
      <section style={{ marginBottom: 'var(--space-xl)' }}>
        ...
        {recentNotes.length === 0 && !loading && (
          <div
            ...
            style={{ marginTop: 'var(--space-md)', background: 'var(--color-primary-light)' }}
          >
      ...
      {dailyReport && (
        <section style={{ marginBottom: 'var(--space-xl)' }}>
          ...
          <div className={styles.statRow}>
            ...
            <StatCard
              variant="purple"
              value={formatTime(dailyReport.total_review_time_ms)}
              label="复习时长"
            />
          </div>
          ...
          {dailyReport.question_type_accuracy.length > 0 && (
            ...
            >
              {dailyReport.question_type_accuracy
                .filter((t) => t.total > 0)
                .map((t) => (
                  <span
                    key={t.question_type}
                  ...
                  >
                    {questionTypeLabels[t.question_type] || t.question_type}: {t.accuracy}% (
                    {t.correct}/{t.total})
                  </span>
                ))}
            </div>
                ...
                <div>
                  ...
                  <span
                    style={{
                      ...
                      background: weakPointBadge.background,
                      color: weakPointBadge.color,
                    }}
                  >
            ...
            >
              {weeklyTrend.map((day) => (
                ...
                >
                  {/* 柱状图 */}
                  <div
                    className={`${styles.trendBar}${day.accuracy < 60 ? ` ${styles.trendBarWarning}` : ''}`}
                    style={{
      ...
      <section>
        ...
        >
          <h2 className="heading-serif" style={{ fontSize: '1.25rem' }}>
            最近笔记
          </h2>
...
function EmailReminderToggle() {
  ...
  useEffect(() => {
    ...
    getUserReminderSettings()
      .then((res) => {
        if (!cancelled) setEnabled(res.email_reminder_enabled);
      })
      .catch(() => {
        /* 容错：读取失败保持未加载状态，不打断页面 */
      });
    return () => {
      cancelled = true;
    };
