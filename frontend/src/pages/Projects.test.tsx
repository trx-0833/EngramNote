...
function addConfirmButton(): HTMLElement {
  const button = screen.getAllByRole('button').find((b) => /^添加\D*\d/.test(b.textContent ?? ''));
...
describe('项目的新建 / 重命名 / 删除', () => {
  it('★ 新建项目：名称为空只提示不发请求；填好后创建并刷新列表', async () => {
    ...
    await userEvent.type(
      screen.getByPlaceholderText(/这个项目是做什么的/),
      '从 Pearl 的三层阶梯读起',
    );
    await userEvent.click(screen.getByRole('button', { name: '创建项目' }));
...
describe('扫描导入', () => {
  ...
  it('扫描失败时显示错误提示，不假装成功', async () => {
    ...
    expect(await screen.findByText('扫描导入失败，请确认后端服务可用后重试')).toBeInTheDocument();
