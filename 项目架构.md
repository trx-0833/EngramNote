...
- **遗忘曲线调度**（✅ 已实现）：
  ...
  - 不同题型使用不同的评分映射策略：选择题多格式匹配(5/1)、填空题三级(5/3/1)、简答题 n-gram 关键词覆盖度映射(1-5)。选择题支持字母前缀匹配（answer="A" vs user="A. 选项文本"）、去前缀纯文本比较等多种格式。
...
│       ├── config.py                # 配置管理（零依赖默认值，含清洗/嵌入配置项）
│       ├── database.py              # SQLAlchemy 异步连接（SQLite/PostgreSQL）
│       ├── models/                  # 数据模型
│       │   ├── base.py              # 基础模型（UUID 主键 + 时间戳）
│       │   ├── user.py              # 用户模型
│       │   ├── note.py              # 笔记模型（含状态机枚举：cleaning/cleaned）
...
│       │   ├── cleaning.py          # 清洗 API（触发/状态/diff/恢复/删除）
│       │   ├── understanding.py     # 理解管道 API（10个端点）
...
│       │   ├── note_service.py      # 笔记业务（分页/搜索/Markdown 读取/清洗版读取）
│       │   ├── storage_service.py   # 存储服务（本地/MinIO 策略模式，os.replace原子移动，第10周优化）
│       │   ├── cleaning_service.py  # 清洗服务（规则清洗/分块/副本生成/恢复/删除）
│       │   ├── embedding_service.py # 嵌入服务（BGE-M3 + Chroma 向量存储）
│       │   ├── llm_service.py       # LLM API 调用服务（httpx直调，debug模式GLM/DeepSeek切换，ConversationSession多轮对话，批量题目生成，RateLimiter令牌桶限流，重试抖动+上限，Semaphore并发控制）
│       │   ├── understanding_service.py # 理解管道服务（章节切分、摘要、知识点提取、卡片去重检测(n-gram)）
│       │   ├── rag_service.py       # RAG 问答服务（向量检索已禁用，使用知识卡片中文n-gram匹配）
│       │   ├── sm2_service.py       # SM-2 间隔重复算法（calculate_sm2核心算法 + quality_from_answer多格式评分映射 + _score_short_answer简答题n-gram评分）
│       │   ├── review_service.py    # 复习调度服务（get_due_quizzes薄弱点优先排序 + submit_answer答题流程+日志 + get_review_stats统计 + get_review_history历史）
│       │   └── report_service.py    # 学习报告服务（get_daily_report今日报告 + get_weekly_trend7天趋势 + get_weak_points薄弱点分析，第9周新增）
│       └── tasks/                   # 异步任务
...
│       │   ├── NoteDetail.tsx       # 笔记详情（三视图：原始版/清洗版/对比视图，统一状态组件）
...
│       └── styles/
│           └── global.css           # 设计系统（CSS 变量 + 组件样式 + diff 视图样式）
├── backend/Dockerfile               # 后端 Docker 镜像（多阶段构建，清华pip源，第12周新增）
