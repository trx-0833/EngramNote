...
2. **业务服务层**（微服务或模块化单体）：
   - **资料处理服务**：接收上传，调用 Mineru / Qwen-ASR 转换为 Markdown。Mineru 与 ASR 已作为子包集成于 backend/app/services/mineru 和 backend/app/services/asr
...
**已实现的笔记版本历史 API 端点**（V2.0 新增）：
- `GET /api/notes/{note_id}/versions` — 获取笔记版本列表（含版本号 / 来源 / 大小 / 创建时间）
- `GET /api/notes/{note_id}/versions/{version_number}` — 预览指定版本内容
- `GET /api/notes/{note_id}/versions/{version_number}/diff` — 与当前版本进行 diff 对比
- `POST /api/notes/{note_id}/versions/{version_number}/restore` — 恢复到指定版本（自动创建当前版本快照）

**已实现的学习目标 API 端点**（V2.0 新增）：
- `GET /api/goals` — 获取用户目标列表（支持 status / goal_type 筛选）
- `POST /api/goals` — 创建学习目标（active 目标数 ≤ 5）
- `GET /api/goals/{goal_id}` — 获取目标详情（含进度）
- `PUT /api/goals/{goal_id}` — 更新目标（标题 / 描述 / 截止时间 / 状态）
- `DELETE /api/goals/{goal_id}` — 软删除目标（status=deleted）
- `GET /api/goals/daily-recommendation` — 获取每日推荐任务（weak_points > review > new_materials）
- `POST /api/goals/refresh-progress` — 手动触发目标进度刷新

**已实现的复习提醒 API 端点**（V2.0 新增）：
- `GET /api/review/reminders` — 获取提醒数据（到期数 / 1 小时内到期 / 薄弱点数）

**已实现的流式问答 API 端点**（V2.0 新增）：
- `POST /api/understanding/ask/stream` — 流式 RAG 问答（SSE 推送：`event: token` / `event: sources` / `event: done` / `event: error`）
...
- **增删查改管理**（✅ 已实现）：
  ...
  - 重复块用 HTML 注释包裹，可随时恢复。
  - 版本历史和 diff 回溯（✅ V2.0 已实现）：编辑 / 自动清洗覆盖前自动创建版本快照，支持列表 / 预览 / diff / 一键恢复。
...
- **智能问答**（✅ V2.0 已完整实现）：RAG 三路混合检索（向量 + BM25 + n-gram）+ RRF 融合；嵌入模型隔离到 Celery Worker 进程解决段错误；支持 SSE 流式输出（`/api/understanding/ask/stream`）
- **总结归纳**：
...
- **遗忘曲线调度**（✅ 已实现）：
  ...
  - 推送方式（✅ V2.0 已实现）：浏览器通知（前端 `notifications.ts`）+ SMTP 邮件提醒（`notification_service.py`）；Celery Beat 每日 09:00 发送提醒邮件；免打扰时段默认 22:00-08:00。
...
- **笔记版本表 (note_versions)**（V2.0 新增）：
  ...
  | source_type | Enum | USER_EDIT / AUTO_CLEAN / SYSTEM |
  | file_path | String(1000) | 版本文件路径 `markdown/{note_id}/versions/v{N}.md` |
  | content_hash | String(64) | 内容哈希（用于去重） |
  | size_bytes | Integer | 版本文件大小 |
  | created_at | DateTime | 自动创建 |

  **保留策略**：USER_EDIT 保留 50 个、AUTO_CLEAN 保留 10 个、SYSTEM 保留 50 个；超出时按 created_at 升序清理最旧版本。

- **学习目标表 (learning_goals)**（V2.0 新增）：
  | 字段 | 类型 | 说明 |
  |------|------|------|
  | id | String (UUID) | 主键 |
  | user_id | String | 外键 → users.id，索引 |
  ...
  | status | Enum | active / completed / expired / archived / deleted |
  | target_count | Integer | 目标任务数（如"今日复习 20 题"） |
  | completed_count | Integer | 已完成数（默认 0） |
  | progress | Float | 进度百分比（0.0-1.0，默认 0.0） |
  | start_date | DateTime | 目标开始时间 |
  | end_date | DateTime | 目标截止时间 |
  | created_at | DateTime | 自动创建 |
  | updated_at | DateTime | 自动更新 |

  **约束**：每用户最多 5 个 active 目标；Celery Beat 每日 00:30 自动刷新进度，过期目标转为 expired。

- **每日计划表 (daily_plans)**（V2.0 新增）：
  | 字段 | 类型 | 说明 |
  |------|------|------|
  | id | String (UUID) | 主键 |
  | user_id | String | 外键 → users.id，索引 |
  | goal_id | String | 外键 → learning_goals.id（nullable） |
  | plan_date | DateTime | 计划日期 |
  | task_type | Enum | weak_points / review / new_materials |
  | priority | Integer | 优先级（3=weak_points / 2=review / 1=new_materials） |
  | target_count | Integer | 任务目标数 |
  | completed_count | Integer | 已完成数（默认 0） |
  | status | Enum | pending / in_progress / completed |
  | metadata_ | JSON | 额外元数据（如关联的卡片 ID 列表） |
  | created_at | DateTime | 自动创建 |
  | updated_at | DateTime | 自动更新 |

  **每日推荐生成规则**：优先级 `weak_points(3) > review(2) > new_materials(1)`，总任务数上限 `DAILY_REVIEW_LIMIT=50`。
...
| 嵌入模型进程隔离（V2.0） | **BGE-M3 加载隔离到 Celery Worker 进程**（`embedding_tasks.py`） | 在 FastAPI 主进程加载 | 主进程加载导致 0xC0000005 段错误，隔离到 Worker 进程彻底解决 |
| RAG 检索策略（V2.0） | **三路混合检索（向量 + BM25 + n-gram）+ RRF 融合** | 纯 n-gram 匹配 | 向量检索修复后启用混合检索；纯 Python BM25（k1=1.5, b=0.75）零新依赖；RRF k=60 |
| LLM 流式输出（V2.0） | **httpx stream=True + SSE** | 同步完整响应 | `chat_stream()` 实现 token 级推送；复用 DeepSeek 缓存（共享 system prompt 前缀） |
| Celery 定时任务（V2.0） | **Celery Beat** | 按需查询替代 Beat | V2.0 引入 Beat：每日 00:30 刷新目标进度、每日 09:00 发送复习邮件 |
| 笔记版本管理（V2.0） | **文件系统版本快照 + DB 元数据** | 仅保留一份副本 | `markdown/{note_id}/versions/v{N}.md` 存储版本文件；DB 记录版本号/来源/哈希；按来源类型差异化保留数量 |
| 复习提醒（V2.0） | **浏览器通知 + SMTP 邮件** | 待实现 | `notification_service.py` 提供数据查询和邮件发送；前端 `notifications.ts` 处理权限/通知/免打扰/sessionStorage 去重 |
...
│   ├── alembic.ini                  # 数据库迁移配置
│   ├── alembic/                     # 迁移脚本（V2.0 新增 004_v2_models.py：note_versions + learning_goals + daily_plans 三张表）
│   ├── data/                        # 运行时数据（自动创建）
...
│       ├── database.py              # SQLAlchemy 异步连接（SQLite/PostgreSQL + _migrate_sqlite Schema迁移 + 孤儿数据清理，V1.2增强；V2.0 _migrate_sqlite 新增 note_versions/learning_goals/daily_plans 三张表防御性建表）
│       ├── core/                    # 核心模块（V1.2新增）
...
│       │   ├── note_version.py      # 笔记版本模型（VersionSource枚举 + NoteVersion表，V2.0新增）
│       │   ├── learning_goal.py     # 学习目标模型（GoalType/GoalStatus枚举 + LearningGoal表，V2.0新增）
│       │   └── daily_plan.py        # 每日计划模型（TaskType/PlanStatus枚举 + DailyPlan表，V2.0新增）
...
│       │   ├── version.py           # 笔记版本 Schema（VersionList/VersionDetail/VersionDiff，V2.0新增）
│       │   ├── goal.py              # 学习目标 Schema（GoalCreate/GoalDetail/DailyRecommendation，V2.0新增）
│       │   └── reminder.py          # 复习提醒 Schema（ReminderData，V2.0新增）
│       ├── api/                     # API 路由
│       │   ├── router.py            # 总路由注册（含清洗+复习+报告+版本+目标+提醒路由）
│       │   ├── auth.py              # 认证 API + get_current_user 依赖
│       │   ├── notes.py             # 笔记 CRUD API（V2.0 新增版本历史子路由）
│       │   ├── upload.py            # 文件上传 API（流式上传，第10周优化）
│       │   ├── cleaning.py          # 清洗 API（触发/状态/diff/恢复/删除，直接操作Note对象无需重复查询）
│       │   ├── understanding.py     # AI 理解管道 API（触发理解/状态/章节/卡片/问答/题目，批量删除旧数据，顶部统一导入；V2.0 新增 ask/stream 流式端点）
│       │   ├── review.py            # 复习调度 API（due/submit/stats/history；V2.0 新增 reminders 端点）
...
│       │   ├── versions.py          # 笔记版本历史 API（列表/预览/diff/恢复，V2.0新增）
│       │   └── goals.py             # 学习目标 API（CRUD+每日推荐+进度刷新，V2.0新增）
...
│       │   ├── cleaning_service.py  # 清洗服务（规则清洗/分块/副本生成/恢复/删除；V2.0 覆盖前自动创建版本快照）
│       │   ├── embedding_service.py # 嵌入服务（BGE-M3 + Chroma 向量存储，模块级单例缓存 + HF_HUB_OFFLINE + 本地缓存预检查，V1.2优化）
│       │   ├── llm_service.py       # LLM API 调用服务（httpx直调，debug模式GLM/DeepSeek切换，ConversationSession多轮对话，批量题目生成，RateLimiter令牌桶限流while循环，重试抖动+上限，Semaphore并发控制；V2.0 新增 chat_stream 流式输出）
│       │   ├── understanding_service.py # 理解管道服务（章节切分、摘要、知识点提取、卡片去重检测(n-gram)）
│       │   ├── rag_service.py       # RAG 问答服务（V2.0 三路混合检索：向量召回 + 纯Python BM25 + n-gram + RRF融合；降级策略：向量失败→BM25+n-gram，全部失败→LLM自身知识）
...
│       │   ├── version_service.py   # 笔记版本历史服务（创建/列表/预览/diff/恢复/清理；按来源差异化保留数量：USER_EDIT 50 / AUTO_CLEAN 10 / SYSTEM 50，V2.0新增）
│       │   ├── goal_service.py      # 学习目标服务（CRUD + 进度计算 + 每日推荐 weak_points(3)>review(2)>new_materials(1) + 上限 DAILY_REVIEW_LIMIT=50 + 每用户≤5 active 目标，V2.0新增）
│       │   └── notification_service.py # 复习提醒服务（提醒数据查询 + SMTP邮件发送 + 免打扰时段22:00-08:00，V2.0新增）
│       └── tasks/                   # 异步任务
│           ├── celery_app.py        # Celery 配置（include: convert_tasks + clean_tasks + understand_tasks + embedding_tasks + reminder_tasks，result_expires=3600结果自动清理；V2.0 新增 Celery Beat 定时：refresh_goal_progress 每日00:30 + send_daily_review_email 每日09:00）
│           ├── convert_tasks.py     # 文档转换任务（app.services.mineru + app.services.asr，转换后自动触发清洗）
│           ├── clean_tasks.py       # 清洗任务（规则清洗→分块→嵌入→去重→生成副本；V2.0 覆盖前自动创建版本快照）
│           ├── understand_tasks.py  # 理解管道任务（章节切分→摘要→知识点提取→题目生成）
│           ├── embedding_tasks.py   # 嵌入任务（V2.0新增，将 BGE-M3 嵌入模型加载隔离到 Celery Worker 进程，避免 FastAPI 主进程段错误）
│           └── reminder_tasks.py    # 提醒任务（V2.0新增，refresh_goal_progress 刷新目标进度 + send_daily_review_email 发送复习邮件）
├── frontend/
...
│       │   ├── VersionHistory.tsx   # 笔记版本历史模态组件（版本列表/diff对比/一键恢复，V2.0新增）
│       │   └── ReminderBanner.tsx   # 复习提醒横幅组件（到期数/1小时内到期/薄弱点数，V2.0新增）
│       ├── utils/
│       │   ├── labels.ts           # 统一标签映射（statusLabels/cardTypeLabels/questionTypeLabels/difficultyLabels等，第11周新增）
│       │   └── notifications.ts    # 浏览器通知工具（权限请求/通知发送/免打扰时段/sessionStorage去重，V2.0新增）
│       ├── pages/
│       │   ├── Login.tsx            # 登录页（使用 useAuth）
│       │   ├── Register.tsx         # 注册页（使用 useAuth）
│       │   ├── Dashboard.tsx        # 仪表盘（最近笔记 + 快捷上传 + 今日待复习卡片 + 今日学习报告 + 薄弱点列表 + 7天趋势柱状图 + 新用户引导；V2.0 集成学习目标卡片和每日推荐）
│       │   ├── NotesList.tsx        # 笔记列表（分页/搜索/删除/归档筛选，统一状态组件）
│       │   ├── NoteDetail.tsx       # 笔记详情（三视图：原始版/清洗版/对比视图，统一状态组件；归档/取消归档后调用 fetchNote 刷新完整数据；V2.0 集成 VersionHistory 模态）
│       │   ├── Upload.tsx           # 上传页（拖拽 + 轮询状态 + 解析方式选择）
│       │   ├── KnowledgeCards.tsx   # 知识卡片列表页（统一状态组件+统一标签映射）
│       │   ├── CardDetail.tsx       # 知识卡片详情页（统一状态组件+统一标签映射）
│       │   ├── QA.tsx               # 智能问答页（V2.0 重构为流式渲染，首字到达后实时显示）
...
│       │   └── LearningGoals.tsx    # 学习目标管理页（目标CRUD+每日推荐+进度展示，V2.0新增）
│       └── styles/
...
**笔记版本历史流程**（V2.0 新增）：
1. 触发时机：用户手动编辑笔记 / 自动清洗覆盖前 / 系统快照（如归档时）。 ✅ 已实现
2. `version_service.create_version()` 将当前 Markdown 内容写入 `markdown/{note_id}/versions/v{N}.md`，并在 `note_versions` 表记录版本号 / 来源 / 哈希 / 大小。 ✅ 已实现
3. 用户在 `VersionHistory.tsx` 模态中查看版本列表，可选择任意版本预览或与当前版本 diff 对比。 ✅ 已实现
4. 一键恢复：将指定版本内容写回清洗副本，并自动为当前内容创建快照（避免恢复操作本身造成丢失）。 ✅ 已实现
5. 清理策略：按来源类型差异化保留（USER_EDIT 50 / AUTO_CLEAN 10 / SYSTEM 50），超出时按 created_at 升序清理最旧版本。 ✅ 已实现

**学习目标管理流程**（V2.0 新增）：
1. 用户在 `LearningGoals.tsx` 创建目标（daily / weekly），系统校验 active 目标数 ≤ 5。 ✅ 已实现
2. `goal_service` 根据目标类型和复习记录计算进度（completed_count / target_count → progress 百分比）。 ✅ 已实现
3. 每日推荐：`GET /api/goals/daily-recommendation` 按 `weak_points(3) > review(2) > new_materials(1)` 优先级生成任务，总上限 `DAILY_REVIEW_LIMIT=50`。 ✅ 已实现
4. Celery Beat 每日 00:30 自动执行 `refresh_goal_progress`：更新进度、将过期目标转为 expired、生成当日 daily_plans。 ✅ 已实现
5. Dashboard 集成目标卡片和每日推荐任务，用户可一键跳转到对应答题 / 学习页面。 ✅ 已实现

**复习提醒流程**（V2.0 新增）：
1. 数据查询：`GET /api/review/reminders` 返回到期数 / 1 小时内到期 / 薄弱点数。 ✅ 已实现
2. 浏览器通知：前端 `notifications.ts` 请求 Notification 权限，根据提醒数据发送系统通知；免打扰时段默认 22:00-08:00；sessionStorage 去重避免重复打扰。 ✅ 已实现
3. `ReminderBanner.tsx` 横幅组件在页面顶部展示提醒摘要。 ✅ 已实现
4. 邮件提醒（可选）：`notification_service.py` 通过 SMTP 发送复习摘要邮件；Celery Beat 每日 09:00 执行 `send_daily_review_email`。 ✅ 已实现

**RAG 混合检索流程**（V2.0 新增）：
1. 用户提问 → `rag_service` 并行执行三路检索：向量召回（Chroma）+ BM25（纯 Python，k1=1.5, b=0.75）+ n-gram 关键词匹配。 ✅ 已实现
2. RRF 融合：`score(d) = Σ 1/(k + rank_i(d))`，k=60，按融合分数排序取 Top-K。 ✅ 已实现
3. 降级策略：向量检索失败（嵌入模型不可用）→ BM25 + n-gram；全部失败 → LLM 自身知识回答。 ✅ 已实现
4. 流式输出：`POST /api/understanding/ask/stream` 通过 SSE 推送 `token` / `sources` / `done` / `error` 事件，前端 QA.tsx 实时渲染。 ✅ 已实现

## 八、架构完善与补充建议

1. ~~**笔记版本与历史**：清洗副本的每次修改（包括AI去重操作、用户手动编辑）都应保存版本，支持 Diff 对比，避免误删重要内容。~~ ✅ V2.0 已实现（NoteVersion 模型 + version_service + VersionHistory.tsx + 4 个 API 端点）
...
5. ~~**学习计划与目标管理**：增加学习目标模块，用户设定"本周完成第三章并掌握所有公式"，系统可拆解任务、推荐每日学习量，并结合复习提醒。~~ ✅ V2.0 已实现（LearningGoal + DailyPlan 模型 + goal_service + LearningGoals.tsx + 7 个 API 端点 + Celery Beat 进度刷新）
...
graph TD
    ...
    Clean -->|覆盖前自动创建版本快照| VersionTb[(note_versions表)]
    ...
    Report --> ReviewLog
    MW --> Search[知识库查询-V2.0混合检索] --> VectorDB
    Search -->|BM25+n-gram+RRF融合| RRF[RRF 融合排序]
    VectorDB --> RRF
    RRF --> LLM[LLM问答-V2.0 SSE流式]
    LLM -->|token/sources/done/error SSE 推送| FE
    MW --> Goal[学习目标服务-V2.0]
    Goal -->|每日推荐+进度刷新| GoalTb[(learning_goals/daily_plans表)]
    Goal -->|目标卡片+每日推荐| FE
    MW --> Notif[复习提醒服务-V2.0]
    Notif -->|到期数/薄弱点| FE
    Notif -->|SMTP 邮件| Mail[邮件服务]
    MQ -->|Celery Beat 00:30 刷新目标进度| Goal
    MQ -->|Celery Beat 09:00 发送复习邮件| Notif
    MQ -->|embedding_tasks 隔离 BGE-M3 加载| VectorDB
```
