...
## 附：依赖漂移的当次结论（2026-09-23，阶段 7.2）

执行：`python backend/scripts/check_dependency_drift.py`（**只读**：不联网、不升级、不改文件）

| 指标 | 数值 |
|---|---|
| 声明条目 | 39（带版本约束 20 / 未锁版本 19） |
| 查到已安装版本 | 39 |
| **在声明范围内** | **11**（原 2 → 6 → **11**，见下方两次对齐的日志）|
| **超出声明范围（漂移）** | **9**（原 18 → 14 → **9**）|
| 声明了却没装（发现） | 0 |
| 传递依赖条目 | 49（其中 **28** 条的约束来自已漂移的父包；原 42 → 38 → 28）|
| 硬依赖缺失 | 0 |

> ⚠️ 上表的"原 2 / 原 18 / 原 42"是 **2026-09-23 首次核查**的数字；
> 两次对齐分别发生在 2026-09-24（6 条）与同日稍后（9 条），日志在小节末尾。

**怎么读这组数字**：它把本文档上文那句"扫描的是一套、跑的是另一套"从叙述变成了
可核对的计数 —— **39 条声明里只有 2 条与真实环境相符**。因此：

1. 本文档记录的漏洞结论（`pip-audit` 19 条 / `npm audit` 11 条）描述的是
   **声明路径**（把 requirements 交给 pip 解析会得到的那一套），
   **不是**本机与 CI 真正在跑的版本集合；
2. "扫描没问题"**不能**推断成"跑着的这套没问题"—— 反过来也一样；
3. 退出码恒为 0 是**有意**的：这些差异是既有事实，把它设成门禁会让 CI 长期常红，
   而常红的门禁只有一个结局（被人加 `|| true`）。要当门禁用 `--fail-on-drift`。

**处置方向**（未做，属独立批次）：让"声明 = CI 安装 = 发布"三者可比 ——
即引入锁文件（`requirements.lock.txt` 或 `uv.lock`），CI 与发布都只装锁定的那一套。
在那之前，本文档的所有扫描结论都应带上"扫的是声明路径"这个限定语。

### 2026-09-24 更新：6 条声明已对齐，漂移 18 → 14

对应 Dependabot PR #5–#10（pip 生态），处理方式是**把声明改成"实际在跑的版本"**，
而不是照抄 PR 里的目标版本 —— 因为 PR 的判据是"上游最新"，本仓库的判据是
"声明 = 现实"。差别体现在 alembic：PR 建议 `~=1.20.0`，而本机跑 1.19.2，
照抄会**制造新的漂移**，因此写 `~=1.19.2`。

已对齐（6 条）：`pydantic` 2.13.5、`pydantic-settings` 2.15.0、`alembic` 1.19.2、
`Pillow` 12.3.0、`pytest-asyncio` 1.4.0、`requests` 2.34.2。

### 2026-09-24 第二次对齐：另外 9 条声明，漂移 14 → 9（在范围内 6 → 11）

对应 Dependabot PR #17–#25（pip 生态，本条日志写作时它们**仍是开放状态**）。
处理方式与上一次**完全一致**：把声明改成"**实际在跑的版本**"，
而不是照抄 PR 里的目标版本。逐条如下（左＝原声明，右＝实装）：

| 包 | 原声明 | 改为 | 实装 | 声明位置 |
|---|---|---|---|---|
| `sqlalchemy[asyncio]` | `~=2.0.0` | `~=2.0.52` | 2.0.52 | `requirements.txt:36` |
| `python-multipart` | `~=0.0.9` | `~=0.0.32` | 0.0.32 | `requirements.txt:41` |
| `python-jose[cryptography]` | `~=3.3.0` | `~=3.5.0` | **3.5.0** | `requirements.txt`（见下）|
| `bcrypt` | `~=4.0.0` | `~=5.0.0` | 5.0.0 | `requirements.txt:47` |
| `httpx` | `~=0.27.0` | `~=0.28.1` | 0.28.1 | `requirements.txt:56` |
| `pypdfium2` | `~=4.30.0` | `~=5.10.1` | 5.10.1 | `requirements.txt:63` |
| `openai` | `~=1.0.0` | `~=2.54.0` | 2.54.0 | `requirements.txt:75` |
| `pyyaml` | `~=6.0.0` | `~=6.0.3` | 6.0.3 | `requirements-pdf-local.txt:20` |
| `torch` | `>=2.0.0` | `>=2.14.0` | 2.14.0+cpu | `requirements-asr.txt:20` |

**一条需要单独说明的事：`python-jose`。** `requirements.txt` 里那句注释原本写着
"python-jose 3.3.x 命中 CVE-2024-33663 / CVE-2024-33664，升级到 >=3.4 是独立批次、
需要单独验证"。但实测：**环境里早就装的是 3.5.0** —— 也就是说
"升级到 >=3.4"这一步**事实上早就发生了**，而声明一直停在 `~=3.3.0`。
本次把声明对齐到 3.5.0（注释也改成了如实的表述）。
⚠️ 这不等于"我们处理了那两个 CVE"：**仍然未做的是迁到 PyJWT**，
那是独立的一件事，别把两者混为一谈。

**指标变化（同一条命令实测）**：

| 指标 | 2026-09-24 第一次对齐后 | 本次对齐后 |
|---|---|---|
| 在声明范围内 | 6 | **11** |
| 超出声明范围（漂移） | 14 | **9** |
| 传递依赖里"约束来自已漂移父包" | 38 | **28** |

**仍未对齐的 9 条**（不在 Dependabot 这次开的 PR 里，属下一轮）：
`fastapi ~=0.115.0`（实装 0.141.1）、`numpy ~=1.24.0`（实装 **2.2.6**，跨大版本）、
`sentence-transformers ~=2.2.0`（实装 3.0.1）、`psutil ~=5.9.0`（实装 7.2.2）、
`uvicorn[standard] ~=0.30.0`（实装 0.52.4）、`celery ~=5.4.0`（实装 5.6.3）、
`aiosqlite ~=0.20.0`（实装 0.22.1）、`email-validator ~=2.0.0`（实装 2.3.0）、
`soundfile ~=0.12.0`（实装 0.14.0）。
另有一条**反向**漂移：`qwen-asr >=0.1.0` 而实装 **0.0.6**（声明比现实"新"）——
这条需要单独判断（是版本号方案变了，还是装错了），不要顺手改。

**口径不变**：以上仍然只是"把声明改成现实"，**没有引入锁文件** ——
所以本文档上文那些扫描结论依然要带"扫的是声明路径"这个限定语（见下一节）。

仍未对齐的 14 条**不是"忘了改"**，而是**改动面超出依赖文件**（需单独评估）：
`fastapi 0.115→0.141`、`celery 5.4→5.6`、`bcrypt 4.0→5.0`、`aiosqlite 0.20→0.22`、
`email-validator 2.0→2.3`、`numpy`、`httpx`、`sentence-transformers` 等 ——
它们要么跨大版本（fastapi/celery/bcrypt 都有行为变更），要么被本项目刻意钉在
兼容区间（例如 `python-jose 3.3.x` 是已知 CVE 的载体，升级需迁移到 PyJWT）。
处置方向仍是那条：**引入锁文件让"声明 = CI 安装 = 发布"三者可比**，而不是逐条追版本。
