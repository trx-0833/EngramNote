...
**3.7 也已完成**（附录 X）。因此**下一个该做的是 3.9**：
`mastery_service.compute_retrievability` 仍在用指数曲线 `2^(-t/S)`，
而 FSRS 用幂律曲线，两者在长间隔上差一个数量级
（S=20 的卡放一年：0.03 vs 0.44）。3.9 现在是**纯粹的一步替换** ——
`stability` 已经在库里，不需要新数据，也不需要新表。
不换的代价是"到期预测"与"掌握度"在界面上互相矛盾。
...
**文档版本**：v1.6（阶段 2 与阶段 3 的执行记录见附录 J–X；FSRS 见附录 W，
到期时刻策略见附录 X）
