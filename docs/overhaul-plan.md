...
| ★ **RAG 检索通道可能降级** | 问答仍能答，但 `retrieval_status` 可能是 `hybrid`（向量命中 0 条，只剩关键词）→ 回答的**依据质量**可能变差，而界面上不明显 | 成因**未定论**：领先假设是**索引时滞**（提问时新 chunk 还没 embedding）；另有一条待测：`rag_service._encode_via_celery` 的 `task.get(10)` **10 秒硬超时**在冷启动下会退回 BM25（但**超时不取消任务**，所以是窗口不是永久）。判定实验：提问前先断言全部 chunk `has_embedding=1` |

### BN.4 只有"代码级证据"、没有实测的两条

| 结论 | 证据强度 |
|---|---|
| "清空字段做不到" | 前端 + 后端三处代码路径**逐行核对**过；**没有**发真实请求复现（所以写"做不到"而不是"实测失败"） |
| "`hybrid` 可能是索引时滞" | **假设**，不是结论；判定方法已写好但没跑 |

---

**文档版本**：v5.9（**2026-09-14 未做事项总表**：新增**附录 BN**，
