...
3. **4.6 Prompt 版本化 + 结构化输出校验**（4.1 遗留的那半条）；
4. **`llm_calls` / `llm_cache` 的清理定时任务**：`purge_expired()` 与
   `llm_call_retention_days` 都写好了，但**没有任何定时任务真的调用它们**，
   长期运行下两张表会持续增长（见 AF.10）；
5. **4.10 `debug` 开关解耦**：拆成 `app_env` / `log_sql` / `llm_provider`。

✅ 已完成：3.5 / 3.6 / 3.7 / 3.9 / 3.12（含前端）/ 3.13 / 3.2 /
4.1（含收尾，附录 AG + AI）/ 4.2 / 4.3 / 4.4 / 4.5 / 4.7 / 4.8 / 4.9 / 4.11。
⏸ 需要真实使用数据：3.10 / 3.11 / 3.14。

**阶段 4 剩下的项**：4.6（Prompt 版本化 + 结构化校验）、4.10（`debug` 解耦）。
...
| # | 动作 | 验收 |
|---|---|---|
| 4.1 | 抽出独立 `LLMGateway`：统一入口、重试/退避/限流/并发/配额/缓存/记账 | `llm_service.py` 从 831 行降到 <300 | ✅ **已落地**（附录 AG + AI）：调用策略 → `services/llm/gateway.py`（625 行），提示词 → `services/llm/prompts.py`（381 行），场景方法 → `services/llm/scenes.py`（556 行）。`llm_service.py` **1273 → 221 行**，对外 API 一行未变（15 个调用方零改动）。⚠️ 结构化输出校验（Pydantic）**未做**，留给 4.6 |
...
**文档版本**：v2.7（阶段 2、3 与阶段 4.1–4.5/4.7–4.9/4.11 的执行记录见附录 J–AI；
