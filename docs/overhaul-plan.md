...
1. **4.4 限流重构** + **4.5 事件循环**：这两项是同一件事的两半 ——
   4.4 要把"进程级 10 RPM"改成按用户/按供应商的令牌桶，4.5 要把
   限流器与信号量改成**每 loop 惰性创建**。计划里给 4.4 写的是 Redis 实现，
   但本项目**单写者已被强制**（`main.py::_enforce_single_writer`），
   进程内按用户的桶就够用，可以先不引入 Redis —— 见 AG.5；
2. **4.1 的收尾**：`llm_service.py` 仍 985 行，距计划的 <300 行还差
   一次**提示词抽取**（纯机械搬移）与 4.11（删调试日志）；
   结构化输出校验（Pydantic）也还没做 —— 它和 4.6 是同一类工作；
3. **Playwright 端到端**（AB.8）—— 前端测试只到组件/页面级，
   而"登录 → 导入 → 复习 → 引用回跳"这条全链路正是历史缺陷最集中的地方；
4. **文档与对外描述的一致性**（§2.1 S-4）：`README.md` / `docs/architecture.md`
   里关于 Vault 目录、端口、行数、alembic 的描述仍是过时的，
   `docs/decisions.md` 尚未转为只读归档 —— **不需要任何数据或新设施**；
5. **4.6 Prompt 版本化**（溯源用，不是 4.7 的前置）。

✅ 已完成：3.5 / 3.6 / 3.7 / 3.9 / 3.12（含前端）/ 3.13 / 3.2 /
4.1（调用策略部分，附录 AG）/ 4.2 / 4.3 / 4.7 / 4.8 / 4.9。
⏸ 需要真实使用数据：3.10 / 3.11 / 3.14。

**阶段 4 剩下的项**：4.4（限流重构）、4.5（loop 安全）、4.6（Prompt 版本化）、
4.10（`debug` 开关解耦）、4.11（删调试日志），以及 4.1 的收尾。
...
| # | 动作 | 验收 |
|---|---|---|
| 4.1 | 抽出独立 `LLMGateway`：统一入口、重试/退避/限流/并发/配额/缓存/记账 | `llm_service.py` 从 831 行降到 <300 | ✅ **已落地（部分）**（附录 AG）：调用策略整体搬到 `services/llm/gateway.py`（538 行），`llm_service.py` **1273 → 985 行**，对外 API 一行未变（15 个调用方零改动）。⚠️ 计划的 **<300 行**未达成：那还需要再做一次纯机械的**提示词抽取**（`services/llm/prompts.py`），以及 4.11 的调试日志删除。结构化输出校验（Pydantic）**未做**，留给 4.6 |
...
**文档版本**：v2.5（阶段 2、3 与阶段 4.1/4.2/4.3/4.7/4.8/4.9 的执行记录见附录 J–AG；
FSRS 见 W，到期时刻策略见 X，掌握度曲线见 Y，前端接线见 Z，
卡片复习 UI 见 AA，前端测试框架见 AB，LLM 记账见 AC，配额见 AD，
卡片入库门见 AE，响应缓存见 AF，LLM 网关见 AG）
