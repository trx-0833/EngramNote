...
**剩下的阶段 3 项目全部卡在"需要真实使用数据"上**：

| 项 | 卡在哪 |
|---|---|
| 3.10 分开限额 | 技术前提已具备（`S`/`R`/`state` 都在库里），但"新卡多少张合适"要靠实测调 |
| 3.11 薄弱点排序加时间衰减 | 同上，排序权重需要真实错误分布 |
| 3.13 复习页展开原文 | **不需要数据**，是纯前端工作 —— 可以做 |
| 3.14 校准曲线 / 参数拟合 | 真库 FSRS 记录数为 **0**，样本还没开始积累 |

**因此下一步该做的是 3.13（前端，不需要数据）**，
以及把 3.5/3.6 已经返回但前端还没展示的东西接上
（`grading_detail` 的"缺了哪一点/误解了哪一点"、S/D/R、`use_semantic_grading` 开关）。
...
**文档版本**：v1.7（阶段 2 与阶段 3 的执行记录见附录 J–Y；
FSRS 见附录 W，到期时刻策略见附录 X，掌握度曲线见附录 Y）
