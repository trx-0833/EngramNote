...
`git diff --exit-code` 退出 0（幂等）。
> **补记（同日晚些时候）**：`/ready` 加了索引积压字段（见 BN.6）之后，产物变成
> `398 998 字节 / 103 / 119 / 185 / 6c6de4ffc9a7`，两条命令仍然退出 0。
> ⚠️ **字节数与 sha256 会随任何一次契约改动失效** —— 这类引用要么写成命令
> （"跑 `dump_openapi.py --check` 看当前值"），要么明确标成"当时的值"。
**没验证到的是 GitHub runner 上那一次真实执行**
...
| 6 | RAG 向量通道：`retrieval_status=hybrid` 的成因 | ✅ **已定论（附录 BN.4，零新运行取证）**：清洗路径**刻意**不写向量（`chunk_service.py:20-28`，要加载 4.3GB 模型），补嵌入只有**人工脚本** `scripts/embed_chunks.py`，Beat 四项任务里**没有**它，任务层也没有"给语料补向量"的任务 → 新笔记进向量通道取决于"有没有人跑脚本"。**待决策**：是否挂 Beat（要先查待嵌入数量，为 0 直接返回，避免每 10 分钟加载一次模型）／是否在 UI 上如实提示"本次只用关键词检索" |
...
## 附录 BN · 未做事项总表：**哪些不影响现在的使用**（2026-09-14）

> 这张表回答一个问题：**还没做的事里，哪些今天用起来感觉不到？**
> 分三档：**甲 = 不影响使用**（做了也未必看得出来，或只在多用户/容器/发布场景才有意义）、
> **乙 = 能正常用，但少一层**（功能在、少一层优化或组织）、
> **丙 = 确实影响使用**（只有两条）。
>
> 判断口径是"**单机、单人、本地**的实际使用路径"（这是本项目当前的部署形态）。
> 每一项都给了依据位置与"什么时候才真的需要做"。

### BN.1 甲：不影响现在的使用

| 事项 | 为什么今天感觉不到 | 什么时候才需要 |
|---|---|---|
| **5.2 TanStack Query** ⏸ | 取数仍是手写 `fetch` + `useEffect`，功能完整、页面照常工作 | 你批准运行时依赖后（计划见 `query-and-state-plan.md`） |
| **5.3 Zustand** ⏸ | 状态靠 context + props 传递，行为正确 | 同上 |
...
| **0.12 / 0.13 的容器化验收** ⛔ | 文件内容是对的（`nginx.conf` 两处关键项 + CI 守卫），只是"容器下可用"没被验证过 | 真跑容器时才能验证 |
| **1.12 / 1.13** ⛔ | **实测证伪后已撤回**（那两个"缺陷"不存在） | 无需做 |
| **挂起三条**：限流按 IP / `requirements.txt` 的 `~=` 策略 / 扫描是否阻断 | 单机单人下与现状等价（附录 BL.5） | 按你的口径挂起，随时可重启 |
| **`uploadRequest` / `askQuestionStream` 的 `ApiError.message` 未统一** | 文案与改造前**逐字一致**（只是两处不前置错误码） | 想统一时——那是行为变更 |
| **3 条"契约有、UI 没传"的查询参数**（`project_id` / `target_categories` / `target_difficulty`） | UI 里没有对应入口，现有流程不经过它们 | 想加"按项目过滤笔记""定向出题"时 |
| **`updateGoal` 前端零调用方** | 只是 API 层多了一个没人调的导出 | 想接目标编辑 UI 时 |
| **未验证项**：P4 的两条 CI 步骤没在 GitHub runner 上跑过、`e2e:full` 会消耗真实 LLM 额度 | 都不影响本机使用 | 推送/发布时 |

### BN.2 乙：能正常用，但少一层

| 事项 | 今天是什么样 | 差的那一层 |
|---|---|---|
| **3.10 新卡与复习卡分开限额** ⏸ | 只有一个"每日复习上限"（`DAILY_REVIEW_LIMIT_REACHED` 那条规则） | 不能分别控制"今天学几张新卡"与"复习几张旧卡" |
| **3.11 薄弱点排序加时间衰减 + overdue 权重** ⏸ | 薄弱点按**错误次数**降序 | "最近错的"不会比"很久以前错的"更靠前 |
| **3.8 leech 检测** 🟡 | 有 `lapses` 字段与相关代码 | 从没度量过效果；"反复忘的卡片该重写"这层提醒没有证据支撑 |
| **5.6 CSS 剩余 6 个样式表** 🟡 | 界面一致（迁移只搬不改值，108 条规则 0 丢失） | 代码组织：14 个全局样式表里还有 6 个没进 CSS Modules |
| **3.14 度量层** 🟡 | 接口与页面都在 | 真库 FSRS 记录为 0 → 恒返回 `insufficient_data=true`（**数据够了自动有值**，不需要改代码） |

### BN.3 丙：**确实影响使用**（只有两条）

| 事项 | 用户会看到什么 | 修法与代价 |
|---|---|---|
| ★ **"清空字段"做不到** | 在项目里把**描述**清空保存 → 看起来保存了，**其实没清掉**（页面重载后旧描述还在）。同样的机制也适用于另外几个可空字段 | 前端 `pages/projects/useProjects.ts:102` 把空值变成 `undefined`（被 `JSON.stringify` 丢掉）；后端 `services/project_service.py:196-199`、`api/understanding.py:465-468`、`api/knowledge.py:180-183` 三处 `if x is not None` 让**显式 `null` 与"不传"等价**。要改服务层语义（区分"显式给了 null"与"没给"），属行为变更 |
| ★ **RAG 检索通道对新笔记降级（`retrieval_status=hybrid`）** | 新上传的笔记在被人工补嵌入之前，问答**只能用 BM25**：换个说法/同义的问题可能答不出或答歪，而界面上看不出来（只有 SSE 的 `meta.retrieval_status` 写着 `hybrid`） | **成因已定论**（见 BN.5）：**不是**超时，也不是泛泛的"索引时滞"，而是**设计分工 + 没有任何自动后续** —— 清洗路径刻意不写向量（要加载 4.3GB 模型），补嵌入只有**人工脚本** `scripts/embed_chunks.py`，且 Beat 调度表里**没有**这个任务 |

### BN.4 RAG 的定向取证（**零新运行**，2026-09-14）

上一轮全链路 e2e 的产物里既有 `retrieval_status`，又留下了**那次运行的临时库**，
于是"为什么降级"可以不开新实验就答完：

| 证据 | 事实 |
|---|---|
| 全链路 evidence（`frontend/test-results-e2e-full/evidence.json`） | 上传 14:23:33 → 清洗完成 → 问答 14:24:21（**48 秒后**），`meta.retrieval_status = "hybrid"` |
| 那次运行的临时库（`%TEMP%\engramnote-e2e-full\db\engram.db`） | 该笔记只有 **1 条** chunk，`embedding IS NULL`、`has_embedding = 0` —— **语料侧根本没有向量**，不是"问题的向量化失败" |
| 设计说明（`app/services/chunk_service.py:20-28`） | 分块只做文本与定位（A 半），**不写向量**（B 半要加载 4.3GB 模型，不能放进清洗流程）；`has_embedding=False` 的行"需由 `scripts/embed_chunks.py` 补嵌入"；`_get_user_chunks` 只取 `has_embedding=True` → **未嵌入的新笔记只被 BM25 检索到，是刻意的降级可用** |
| Beat 调度表（`app/tasks/celery_app.py:128` 起） | 四项：目标进度刷新 / 复习提醒邮件 / 僵尸任务自愈 / 每日备份 —— **没有**补嵌入 |
| 任务层（`app/tasks/embedding_tasks.py`） | 只有 `encode_text`（给**问题**、图谱建议编码），**没有任何"给语料 chunk 补向量"的任务**；`scripts/embed_chunks.py` 是本地加载模型直接算 |
| 真库（只读查询，mtime 未变） | 608 条 chunk **全部**有向量（0 条 NULL）；但 `created_at 04:47:36` → `updated_at 05:12:41`（**晚 25 分钟**）—— 正是"先入库、事后批量补"的指纹 |

**结论**：`hybrid` 对"刚导入的笔记"而言是**预期状态**，不是故障；真正的缺口是
**补嵌入没有任何自动路径**（没有 Beat、没有随清洗触发的任务、UI 也不提示），
所以"新笔记什么时候进向量通道"取决于**有没有人记得跑那个脚本**。
...
| 结论 | 证据强度 |
|---|---|
| "清空字段做不到" | 前端 + 后端三处代码路径**逐行核对**过；**没有**发真实请求复现（所以写"做不到"而不是"实测失败"） |

### BN.6 收尾：把"待补向量"变可见（后端 `/ready` + 前端提示）

BN.4 定论之后，"补嵌入没有自动路径"这件事有两种修法：挂定时任务，或先让它**可见**。
选后者 —— 挂 Beat 要定频率与内存（4.3GB 模型 × 与用户路径抢资源），那是决策；
而"看不见"本身就是缺陷，且修起来不需要决策。

| 项 | 内容 |
|---|---|
| 后端 | `GET /ready` 新增 **`index: {pending_embeddings, source}`**：数 `chunks.has_embedding = false` 的行（只读、单次 count、走 `ix_chunks_user_embed` 的列） |
| 为什么只报告不阻断 | 与队列深度**同一条判据**：积压说明检索质量暂时降级，不说明实例坏了。若把它做成就绪判据，"刚导入、还没补向量"会让实例被判未就绪 —— 那是把"慢/降级"错当成"坏" |
| `None` 而不是 0 | 库不可用时回 `None` + 原因码 `database_unavailable`，与 `QueueDepth.depth` 同一口径（0 会让"没测"与"真的没积压"在面板上长得一样） |
| 测试 | `tests/test_ready_endpoint.py` **16 → 19**：形状固定 / **只数未嵌入的行**（3 条已嵌入 + 2 条未嵌入 ⇒ 报 2）/ **有积压仍 200**；另在"库不可用"用例里补了 `None` 的断言 |
| 契约产物 | `openapi.json` → **398 998 字节 / 103 / 119 / 185 / `6c6de4ffc9a7`**（新增 `IndexBacklog` 组件）；`schema.ts` 同步再生，漂移逐项不变（`pathMatched 111 / identical 105 / 54 / 22 / bodyChecked 34`） |
| 前端（同一轮） | 新增 `utils/retrievalNotice.ts`：把 SSE `meta.retrieval_status` 翻成一句话（`hybrid` / `bm25_only` 各一句；`full_vector` 与未知值**不提示**）。QA 页与**笔记内提问**都接上 —— 修复前只有 `bm25_only` 有提示，而实际发生的是 `hybrid`，且笔记内提问**连 `retrieval_status` 都没取** |
| 用例与变异 | `retrievalNotice.test.ts` 5 例（含"`bm25_only` 文案逐字不变"与"`hybrid` 文案不断言单一成因"）；`NoteAskPanel.test.tsx` 3 → **5** 例（喂一段真 SSE，断言提示被渲染）。**变异验证**：把渲染那三行注掉后新用例立刻失败（`1 failed \| 4 passed`），还原后通过 |
...
**推送本身被网络挡住**：`github.com:443` 不通（DNS 正确、`api.github.com` 等其它主机正常），
SSH 端口可达但本机无密钥、无代理 —— 三条解封路径写在 BO.4。
README 的"文档"一节重写成入口 / 重构记录 / **质量门禁表** / 历史四段）；
v5.13 —— **2026-09-14 5.6 收官确认 + 首次推送**：核查发现样式表迁移**早已完成**，
"剩 6 个"是把"文件还在"读成了"活没干"（`base.css` 令牌层永不迁、4 份保留刻意全局的规则、
5 份只剩注释却因 `mobile-input-font-size.test.ts` 的断言不能删），见**附录 BO**；
...
前端新增 `utils/retrievalNotice.ts` 并接到 QA 页与**笔记内提问**两处 ——
修复前只有 `bm25_only` 有提示、而实际发生的是 `hybrid`，笔记内提问连这个字段都没取，
见**附录 BN.6**；契约产物随之变为 `398 998 字节 / 103 / 119 / 185 / 6c6de4ffc9a7`，
并记下"指纹类引用会随每次契约改动失效"这条口径）；
v5.10 —— **2026-09-14 RAG `hybrid` 成因定论**：用上一轮全链路留下的
**临时库 + evidence.json** 做定向取证（**零新运行、零 LLM 消耗**）——
不是超时、也不是泛泛的"索引时滞"，而是**设计分工 + 无任何自动后续**：
清洗刻意不写向量、补嵌入只有人工脚本、Beat 里没有该任务（附录 BN.3 / BN.4）；
同时把"清空字段"的登记收进 BN.5。**我此前给的两个假设（10 秒超时 / 索引时滞）
都不成立，已在文里明确更正**）；
v5.9 —— **2026-09-14 未做事项总表**：新增**附录 BN**，
把"还没做的事"按 **甲 不影响使用 / 乙 少一层 / 丙 确实影响使用** 三档列表，
每项给依据位置与"什么时候才需要做"；结论是**丙档只有两条**
（项目描述清空不生效、RAG 检索通道可能降级），其余都不影响单机单人本地使用）；
v5.8 —— **2026-09-14 阶段 5.1 的 S3b**：18 个函数的查询串接入契约
