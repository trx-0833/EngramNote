...
3. **阶段 4 的余项**：4.4（限流重构，需要 Redis）、4.6（Prompt 版本化）、
   4.7（响应缓存）—— 其中 4.6 / 4.7 不需要新基础设施；
4. **4.1 LLMGateway**：`llm_service.py` 已 1187 行，且 4.2/4.3 又往里加了
   约 90 行。**越晚做越贵**，应当尽快排上。
...
**阶段 4 剩下的项**：4.1（网关重构，最大）、4.4（限流重构，要 Redis）、
4.6（Prompt 版本化）、4.7（响应缓存）。其中 **4.6 与 4.7 都建立在
"能识别同一份输入"这件事上** —— 4.8 刚加的 `content_hash` 是现成的基础。
...
**文档版本**：v2.3（阶段 2、3 与阶段 4.2/4.3/4.8/4.9 的执行记录见附录 J–AE；
