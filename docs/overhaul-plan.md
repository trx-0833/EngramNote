...
| # | 动作 | 目的 | 状态 |
|---|---|---|---|
| 2.2′ | `chunks` 表（纯 SQLite，无扩展）承载 chunk 文本 + `char_start/char_end/heading_path` + 向量 BLOB | 支撑 2.7 回跳与 2.3 语料切换 | 🟡 **A 半已落地**（附录 O）：表已建、**608 个 chunk / 20 篇笔记**已落库并校验（不变量 + 覆盖率全绿、幂等）。**B 半（向量）待做** —— 受内存限制，见 N.5 |
| 2.5′ | **FTS5** 全文索引（SQLite 内置）替换纯 Python BM25 | 免维护索引；这是 A-5 在 SQLite 路线下的正解 | ⬜ 待做（`chunks` 表已就位，文本来源已有） |
| 2.4′ | 评估 `sqlite-vec` 扩展替代 Chroma 的多 collection | 消除"每篇笔记一个 collection、检索要遍历全部" | ⬜ 待做（**现状比预想更差**：索引已碎成三种模型，43% 向量取不到 —— 附录 N） |
...
**顺序约束（已按实测修正一次）**：`2.9 → 2.1 → 2.2′ → 2.3/2.4 → 2.7`。

原约束是 `2.1 → 2.7 → 2.3`。执行到 2.7 时实测发现**它做不了**：
位置信息在检索链路四层里丢了三层，且 BM25 通道的卡片语料**根本没有原文位置**
（见附录 N.4）。因此 2.7 的前置是 **2.2′（`chunks` 一等实体）**，不是 2.1。

现状：2.9 ✅、2.1 ✅ → **下一个动作是 2.2′**。
...
- **2.2′ B 半**：向量列（受内存限制，需明确决定）
- **2.3** 语料切换到 chunk 语料（`chunks` 表已就位）
- **2.7 接线**：各通道返回定位字段 + `AnswerSource` + 前端跳转（建议与 2.3 一起）
- **2.5′** FTS5、**2.4′** `sqlite-vec` 评估
