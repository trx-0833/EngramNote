...
| 🟡 部分完成 | 5.12 复习 UI（卡片复习页已重做）、5.13 测试（Vitest 61 例已进 CI 阻断；Playwright 缺） |
| ⏸ 可自主推进（无需新依赖，量大） | 5.5 拆分巨型页面（`NoteDetail.tsx` 1178 行）、5.9 可访问性、5.10 响应式 |
| ⚠️ 需先确认 | 5.1 OpenAPI 代码生成、5.2 TanStack Query、5.3 Zustand、5.6 CSS 体系重建（选型即决策）、5.13 Playwright（浏览器二进制 + 磁盘） |
...
| 5.10 | **响应式**：`responsive.css` 只有 41 行 → 补齐移动端布局 | 手机可用 | ⏸ **未做**（实测 `responsive.css` 现为 51 行，仍远不足以覆盖移动端） |
...
| 5.12 | **复习 UI 重做**：四档自评 + 答错展开原文 + 引用回跳 | 核心流程体验 | 🟡 **部分落地**（阶段 3.12 / 附录 AA：卡片复习页、四档自评、原文折叠；引用回跳见附录 Z 的 `citationJump`）。剩余：答题复习页与卡片复习页的统一 |
| 5.13 | **Vitest + Playwright**：核心流程 E2E（上传→理解→复习→问答） | CI 里有 E2E | 🟡 **Vitest 已落地**（附录 AB，61 个用例并进 CI 阻断）；**Playwright 未做** —— 需下载浏览器二进制（数百 MB，D 盘可用 10 GB），**需确认** |
...
| 6.6 | 备份与恢复：定时 `pg_dump` + 对象存储快照 + 一键恢复演练 | 演练成功 | ✅ **已落地**（附录 AS）：备份侧**早已具备**（`VACUUM INTO` 原子快照 + 落盘即校验 + 保留策略 + Beat 每日 03:30 + `scripts/restore_db.py`）；本轮补上**恢复演练**（`inspect_snapshot` / `run_restore_drill` + 每周一 05:00 的 Beat 任务）。⚠️ 计划写的 `pg_dump` 不适用（本项目是 SQLite），改用等价能力。真备份实测：**演练通过**，仅 1 条"悬挂外键"warning（已知有意保留） |
...
**文档版本**：v3.9（阶段 2、3、阶段 4 全部，阶段 5 的 5.11 与 5.5 前置，
阶段 6 的 6.1 / 6.2 / 6.4 / 6.5 / 6.6 / 6.8 见附录 J–AU；
...
恢复演练见 AS，密码策略与登录时序见 AT，限流覆盖见 AU）
