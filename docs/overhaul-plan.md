...
| ✅ 已完成（5.9 的修复一半除外） | 5.5 拆分巨型页面（三页全部拆完，附录 AX/AZ/BB）、5.10 响应式（附录 BC）**已完成**；5.9 可访问性**审计已完成、修复进行中**（附录 BG） |
...
| 5.9 | **可访问性**：键盘导航、focus trap（模态）、ARIA、对比度 | axe 无 critical | 🟡 **部分落地（审计已完成，修复进行中）**：审计一半见**附录 BG** —— axe-core 4.13 + 真 Chromium（`npm run a11y`，10 条用例覆盖 10 个场景；非空夹具、未定义 `/api` 一律 501、"axe 真的跑了"四道断言），CI 里**顾问式**接入（`continue-on-error: true`，升级条件写在 job 注释里）。⚠️ **计划的原验收"axe 无 critical"当场不成立**：实测 **30 组 / 68 节点**违规（critical **2** / serious 39 / moderate 21 / minor 6），另有 **149 个节点 axe 明确不下结论**（渐变背景）—— **应用不是无障碍合格的**；而"没报出来"也不等于"没问题"（Tab 顺序、屏幕阅读器语义、动态区域它都看不见，且**根本没有触控目标尺寸规则**）。逐条发现与归属见 `frontend/docs/a11y-audit.md`，**修复轮正在进行**（改动在 `frontend/src/**`） |
...
**文档版本**：v5.1（阶段 2、3、阶段 4 全部，阶段 5 的 5.5 / 5.10 / 5.11 / 5.13
与 5.9 的审计一半，阶段 6 的 6.1 / 6.2 / 6.3 / 6.4 / 6.5 / 6.6 / 6.8 见附录 J–AV 与 BA；
...
真库第二轮清理与依赖漂移可见化见 BF，可访问性审计见 BG）
