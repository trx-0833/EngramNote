...
| **RRF 加权（原 2.6 遗留）** | ⬅️ **下一项** | 实测融合比最好单通道低 **28 个百分点**（附录 P.4）—— 先修它 |
| 2.3 语料切换正式接线 | ⬜ | 向量已就位，差接进 `retrieve_context` |
| 2.7 引用回跳接线 | ⬜ | 定位字段已随 chunk 返回，差贯通到 `AnswerSource` 与前端 |
| 2.5′ FTS5 | ⬜ | 独立，可随时插入 |

**顺序再次修正为**：`2.9 ✅ → 2.1 ✅ → 2.2′ ✅ → 2.4′ ✅ → RRF 加权 → 2.3 → 2.7`。

第二次修正的理由：原计划把"RRF 加权"当成 2.6 的一个可选尾项（"RRF 改为
加权融合，向量/BM25 可配权重"），并注明"权重须由评测给出依据"。
现在评测给出了依据，而且结论比预期严重 —— **当前未加权的 RRF 正在让
检索变差约 28 个百分点**。在修掉它之前做 2.3/2.7 会把一个已知有害的
融合逻辑继续留在链路上，且会掩盖 2.3 的真实效果。
...
### P.4 🔴 RRF 融合比任一单通道都差（−28 个百分点）

**这是本轮最重要的发现**，且与预期相反 —— 融合本应"取长补短"：
...
"任一命中即算命中"的上界 = `(488+24+144)/1058` = **62.0%**，
而融合只做到 **32.0%** —— 也就是说，融合**把大量本已命中的结果挤掉了**。

**机制**：RRF 的权重是 `1/(k+rank)`。`k=60` 时：

```
第 1 名: 1/61 = 0.01639
第 5 名: 1/65 = 0.01538     相差仅 4%
```

名次差异被 `k=60` 压得几乎不存在，于是分数主要由**"是否两路同时出现"**决定
（同时出现得 `2/61 ≈ 0.0328`，约等于单路第 1 名的两倍）。
RRF 实际在奖励**共识**而不是相关性。当两路强弱悬殊
（此处 59.74% vs 48.39%），共识偏向就等价于**把强通道拉向弱通道**：
BM25 独有命中的 144 条被大量牺牲，换来的是向量通道的噪声也进入前列。

**这是"照搬默认参数"的代价** —— §2.14 A-14 早就指出
"RRF 的 `k=60` 是照搬的默认值，没有针对场景调参"，当时只是"待调参"的
待办事项；现在实测表明它**正在造成实际损害**。
...
测试总数仍为 **448 passed / 3 skipped**（本附录未新增代码测试；
`embed_chunks.py` 与 `chunk_search_service.py` 的测试待补 —— 见 P.7）。
...
- **RRF 加权/替换**（P.4，已提升为下一项）
- **2.3** 语料切换正式接进 `retrieve_context`
- **2.7 接线**：定位字段贯通到 `AnswerSource` 与前端
- **2.5′** FTS5
- `embed_chunks.py` / `chunk_search_service.py` 的**测试**（本轮以实测数据
  验证，但缺少可重复的回归测试）
