...
| **卡片复习的前端**（3.12 的另一半） | **不需要数据**，是纯前端工作 —— 但它是新发现的一项，不在原表里 |

**因此下一步该做的是"卡片复习 UI"**（后端接口齐备、无数据依赖），
或者按 L-2 的原始判断补 `docs/` 与 README 的过时描述。
...
**文档版本**：v1.8（阶段 2 与阶段 3 的执行记录见附录 J–Z；
FSRS 见附录 W，到期时刻策略见附录 X，掌握度曲线见附录 Y，前端接线见附录 Z）
