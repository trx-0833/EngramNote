...
| 🟡 部分完成 | 5.12 复习 UI（卡片复习页已重做）、5.13 测试（Vitest 已进 CI 阻断；**Playwright 已落地并进 CI 阻断**，见附录 AB/BE/BF.6） |
| ⏸ 可自主推进（无需新依赖，量大） | 5.5 拆分巨型页面（`NoteDetail.tsx` 1178 行）、5.9 可访问性、5.10 响应式 |
| ⚠️ 需先确认 | 5.1 OpenAPI 代码生成、5.2 TanStack Query、5.3 Zustand、5.6 CSS 体系重建（选型即决策）（**5.13 的 Playwright 已落地并接入 CI**，见 BF.6） |
...
| 5.13 | **Vitest + Playwright**：核心流程 E2E（上传→理解→复习→问答） | CI 里有 E2E | 🟡 **Vitest 已落地**（附录 AB，61 个用例并进 CI 阻断）；**Playwright 已落地并接入 CI（阻断性）**（附录 BE.3 + BF.5/BF.6）：4 个 spec / **10 passed / 0 skipped**，并当场抓到一条真缺陷（从 `/login` 登录成功落 404，已修）。⚠️ 覆盖的仍是"外壳 + 认证入口"级别的冒烟：计划字面的"上传→理解→复习→问答"**没有做**，边界见 `frontend/docs/e2e.md` §4 |
...
**文档版本**：v5.0（阶段 2、3、阶段 4 全部，阶段 5 的 5.11 与 5.5 前置，
