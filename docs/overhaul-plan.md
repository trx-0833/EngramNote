...
**因此现在的下一步**（阶段 3 已无"不需要数据"的剩余项）：

1. **Playwright 端到端**（AB.8）—— 本附录只到组件/页面级，
   而"登录 → 导入 → 复习 → 引用回跳"这条全链路正是历史缺陷最集中的地方；
2. **阶段 4 的一部分（LLM 网关与成本治理）** —— 其中 4.2「`llm_calls` 记账表」
   与 4.8「抽取幂等」不需要用户数据即可设计与验证；
...
**文档版本**：v2.1（阶段 2、3 与阶段 4.2 的执行记录见附录 J–AC；
FSRS 见 W，到期时刻策略见 X，掌握度曲线见 Y，前端接线见 Z，
卡片复习 UI 见 AA，前端测试框架见 AB，LLM 记账见 AC）
