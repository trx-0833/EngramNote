...
1. **4.1 的收尾**：`llm_service.py` 仍 985 行，距计划的 <300 行还差
   一次**提示词抽取**（纯机械搬移）与 4.11（删调试日志）；
   结构化输出校验（Pydantic）也还没做 —— 它和 4.6 是同一类工作；
2. **Playwright 端到端**（AB.8）—— 前端测试只到组件/页面级，
   而"登录 → 导入 → 复习 → 引用回跳"这条全链路正是历史缺陷最集中的地方；
3. **文档与对外描述的一致性**（§2.1 S-4）：`README.md` / `docs/architecture.md`
   里关于 Vault 目录、端口、行数、alembic 的描述仍是过时的，
   `docs/decisions.md` 尚未转为只读归档 —— **不需要任何数据或新设施**；
4. **4.6 Prompt 版本化**（溯源用，不是 4.7 的前置）；
5. **`llm_calls` / `llm_cache` 的清理定时任务**：`purge_expired()` 与
   `llm_call_retention_days` 都写好了，但**没有任何定时任务真的调用它们**，
   长期运行下两张表会持续增长（见 AF.10）。

✅ 已完成：3.5 / 3.6 / 3.7 / 3.9 / 3.12（含前端）/ 3.13 / 3.2 /
4.1（调用策略部分，附录 AG）/ 4.2 / 4.3 / 4.4 / 4.5 / 4.7 / 4.8 / 4.9。
⏸ 需要真实使用数据：3.10 / 3.11 / 3.14。

**阶段 4 剩下的项**：4.6（Prompt 版本化）、4.10（`debug` 开关解耦）、
4.11（删调试日志），以及 4.1 的收尾。
...
**文档版本**：v2.6（阶段 2、3 与阶段 4.1–4.5/4.7/4.8/4.9 的执行记录见附录 J–AH；
