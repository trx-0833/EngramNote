...
| # | 动作 | 目的 | 状态 |
|---|---|---|---|
| 2.2′ | `chunks` 表（纯 SQLite，无扩展）承载 chunk 文本 + `char_start/char_end/heading_path` + 向量 BLOB | 支撑 2.7 回跳与 2.3 语料切换 | 🟡 **A 半已落地**（附录 O）：表已建、**608 个 chunk / 20 篇笔记**已落库并校验（不变量 + 覆盖率全绿、幂等）。**B 半（向量）与 2.3 是同一次操作**，见上方"关键依赖" |
| 2.5′ | **FTS5** 全文索引（SQLite 内置）替换纯 Python BM25 | 免维护索引；这是 A-5 在 SQLite 路线下的正解 | ⬜ 待做（`chunks` 表已就位，文本来源已有）**不依赖嵌入模型，可随时插入** |
| 2.4′ | 评估 `sqlite-vec` 扩展替代 Chroma 的多 collection | 消除"每篇笔记一个 collection、检索要遍历全部" | ⬜ 待做（**现状比预想更差**：索引已碎成三种模型，43% 向量取不到 —— 附录 N）**与 2.2′ B / 2.3 共用同一批向量** |

> **2.2′ 的验收标准**（按附录 N 扩充，当前 A 半满足第 1、3 条）：
> 1. ✅ `chunks` 表按 `note_id` 落库 chunk 文本 + `char_start/char_end/heading_path`
> 2. ⬜ **全部向量由同一个模型生成、维度一致、100% 可检索**
>    （当前 1190 条里只有 681 条可取到，见 N.2；B 半完成时校验）
...
| 2.2′ **A 半**（表 + 偏移落库） | ✅ | 608 个 chunk 已落库并校验（附录 O） |
| 2.2′ **B 半**（向量列） | ⬜ | **受内存限制**（附录 N.5） |
| 2.3 语料切换 | ⬜ | 依赖 B 半的同一批向量 |
| 2.7 接线 | ⬜ | 依赖 2.3（语料换成 chunk 后接线更短） |
| 2.5′ FTS5 / 2.4′ `sqlite-vec` | ⬜ | 独立，不依赖上面的向量决定 |

**关键依赖：2.2′ B 半与 2.3 是同一件事的两半** —— 都要"把 chunk 用**一个**
模型重新嵌入"。区别只是写到哪里（`chunks.embedding` 列 / 新的统一索引）。
它们是**一次操作**，不该分两次做：

- 分两次 = 把 608 个 chunk 嵌入两遍（内存吃紧时尤其不该）
- 合一次 = 嵌入一遍，同时满足 2.2′ 验收第 2 条与 2.3 的语料切换

因此**下一步不是二选一，而是先做那个共同的前置决定**：用哪个模型、
以及向量落到哪里（见下）。该决定之后，2.2′ B + 2.3 + 2.4′ 可以一次做完。

#### 待决定的唯一分叉：嵌入模型

| 选项 | 模型大小 | 后果 |
|---|---|---|
| `bge-m3`（当前默认） | 4356 MB | 检索质量最好（上限达成率 83.2%）；但可用内存 4.51GB / 门槛 4.0GB，**余量 0.5GB**，中途 OOM 会留下半新半旧索引 |
| `bge-small-zh-v1.5` | 183 MB | 稳定、快；检索质量较低（上限达成率 60.4%），且需显式改 `EMBEDDING_MODEL` 配置 |
| 暂缓向量、先做 2.5′ | — | FTS5 不碰模型，可立即推进；但 2.2′ 验收第 2 条与 2.3 都继续挂起 |

**注意当前库里两种模型各有一半**（bge-small 509 条、bge-m3 204 条、无记录 477 条，
见上表），所以"保持现状"不是一个选项 —— 现状就是 43% 向量取不到。
