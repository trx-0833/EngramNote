...
**顺序约束（现已满足前两步）**：`2.1 → 2.7 → 2.3`，且 **2.9 先于 2.3/2.4** ——
否则"卡片语料换成原文语料"无法判断是变好还是变差。
现状：2.9 ✅、2.1 ✅、2.7 **后端 ✅ / 前端待做** → **下一个动作是 2.7 前端**。

#### 一个尚未解决的方法论问题：向量通道没有被评测覆盖

`scripts/eval_retrieval.py` 目前**只测 BM25（词法）通道**。附录 K/J/M 的所有
数字都是词法通道的。这不是疏漏，而是被两个事实卡住：

1. 向量检索经 Celery + BGE-M3（进程隔离是为了避免主进程段错误），
   离线脚本要复现就得加载 2.3GB 模型并起 worker；
2. **线上是 `RRF(向量, BM25)` 融合**，只测其中一路无法给出端到端结论。

后果要说清楚：现在可以判断"换语料对**词法**检索更好"（已实测，附录 M.5.1），
但**无法判断**换语料对**向量**检索、以及最终融合结果的影响。
而 2.3 的动机之一正是 A-2（"检索语料是 LLM 摘要而非原文"）——
它主要影响的是**语义**检索。

因此 2.3 的验收标准需要补一条：**向量通道必须纳入评测**，
否则这次改动只被验证了一半。做法是给 `eval_retrieval.py` 增加
`--channel vector|hybrid`（复用 Chroma 里已嵌入的向量，避免重新嵌入）。
